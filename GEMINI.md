...
12. **On receiving `worker_done` from a builder, decide whether to spawn a reviewer or self-verify based on task complexity.**
    ...
    **Reviewer verification (complex tasks):**
    ...
    ov mail send --to review-<builder-name> \
      ...
      --type dispatch
    ```
    The reviewer validates against the builder's spec and runs the project's quality gates (`bun test`, `bun run lint`, `bun run typecheck`).
...
3. Run integration tests if applicable: `bun test`, `bun run lint`, `bun run typecheck`.
...
- **Agent Name:** lead-overstory-6e7a
- **Task ID:** overstory-6e7a
- **Spec:** No spec file provided
- **Branch:** overstory/lead-overstory-6e7a/overstory-6e7a
- **Worktree:** /home/iamzubin/Work/overstory/.overstory/worktrees/lead-overstory-6e7a
- **Parent:** coordinator
- **Depth:** 0
...
Your worktree root is: `/home/iamzubin/Work/overstory/.overstory/worktrees/lead-overstory-6e7a`

**CRITICAL**: All file operations MUST use paths within this directory.
- Use paths relative to your worktree root, or absolute paths starting with `/home/iamzubin/Work/overstory/.overstory/worktrees/lead-overstory-6e7a`
...
These paths are relative to your worktree root: `/home/iamzubin/Work/overstory/.overstory/worktrees/lead-overstory-6e7a`
...
Use `ov mail` for all communication. Your address is **lead-overstory-6e7a**.

```bash
# Check your inbox (do this regularly)
ov mail check --agent lead-overstory-6e7a

# Send a status update to your parent
ov mail send --to coordinator --subject "status" \
  --body "Progress update here" --type status --agent lead-overstory-6e7a

# Ask a question
ov mail send --to coordinator --subject "question" \
  --body "Your question here" --type question --priority high --agent lead-overstory-6e7a

# Report completion
ov mail send --to coordinator --subject "done" \
  --body "Summary of what was done" --type result --agent lead-overstory-6e7a

# Reply to a message
ov mail reply <message-id> --body "Your reply" --agent lead-overstory-6e7a
```
...
ov sling <task-id> --capability builder --name <worker-name> \
  --parent lead-overstory-6e7a --depth 1
```
...
4. **Commit:** all changes committed to your branch (overstory/lead-overstory-6e7a/overstory-6e7a)
5. **Record mulch learnings:** `ml record <domain> --type <convention|pattern|failure|decision> --description "..." --outcome-status success --outcome-agent lead-overstory-6e7a` — capture insights from your work
6. **Signal completion:** send `worker_done` mail to coordinator: `ov mail send --to coordinator --subject "Worker done: overstory-6e7a" --body "Quality gates passed." --type worker_done --agent lead-overstory-6e7a`
7. **Close issue:** `sd close overstory-6e7a --reason "summary of changes"`
...
- **WORKTREE ISOLATION**: All writes MUST target files within your worktree at `/home/iamzubin/Work/overstory/.overstory/worktrees/lead-overstory-6e7a`
- NEVER write to the canonical repo root — all writes go to your worktree copy
- Only modify files in your File Scope
- Commit only to your branch: overstory/lead-overstory-6e7a/overstory-6e7a
